...
async def cadastrar_transacao_bancaria(
    ...
    conta_origem = await ContaBancariaRepoDominio().consultar_por_numero_da_conta(
        numero_da_conta=transacao_bancaria.numero_da_conta
    )
    if not conta_origem:
        raise ContaBancariaNaoEncontrado

    if transacao_bancaria.numero_da_conta_destino:
        conta_destino = await ContaBancariaRepoConsulta().consultar_um_por_filtros(
    ...
    id_resultado = await TransacaoBancariaRepoDominio().adicionar(
